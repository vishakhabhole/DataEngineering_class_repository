-- =============================================================
-- Problem 3 Medium — Running Total
-- Pattern: SUM OVER (PARTITION BY ORDER BY) — resets per group
-- =============================================================
-- Question:
--   Show each order with a running total of amount partitioned
--   by customer_id (the total resets for each customer).
--   Return: customer_id, order_id, order_date, amount, running_total.
--   Order by customer_id, order_date.
-- =============================================================

DROP TABLE IF EXISTS orders;

CREATE TABLE orders (
    order_id    SERIAL PRIMARY KEY,
    customer_id INT,
    order_date  DATE,
    amount      NUMERIC(10,2)
);

INSERT INTO orders (customer_id, order_date, amount) VALUES
    (1, '2024-01-05', 250.00),
    (1, '2024-01-12', 180.00),
    (1, '2024-01-20', 320.00),
    (1, '2024-02-03', 150.00),
    (2, '2024-01-08', 400.00),
    (2, '2024-01-22', 275.00),
    (2, '2024-02-10', 190.00),
    (3, '2024-01-15', 500.00),
    (3, '2024-01-30', 225.00);

-- Expected output:
--
--   customer_id | order_id | order_date | amount | running_total
--   ------------+----------+------------+--------+--------------
--             1 |        1 | 2024-01-05 | 250.00 |        250.00
--             1 |        2 | 2024-01-12 | 180.00 |        430.00
--             1 |        3 | 2024-01-20 | 320.00 |        750.00
--             1 |        4 | 2024-02-03 | 150.00 |        900.00
--             2 |        5 | 2024-01-08 | 400.00 |        400.00
--             2 |        6 | 2024-01-22 | 275.00 |        675.00
--             2 |        7 | 2024-02-10 | 190.00 |        865.00
--             3 |        8 | 2024-01-15 | 500.00 |        500.00
--             3 |        9 | 2024-01-30 | 225.00 |        725.00

-- YOUR ANSWER:
